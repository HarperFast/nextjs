...
- The cluster suite (`integrationTests/cluster.pw.ts`) needs Docker and a **harper-pro** image. OSS `harper` has no replication module and rejects `add_node` as an unknown operation, so it cannot form a cluster. Override the image with `HARPER_PRO_IMAGE`; the suite skips itself when Docker or the image is missing. `HARPER_CLUSTER_KEEP=1` leaves the containers up for inspection.
