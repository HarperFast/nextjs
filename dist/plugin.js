...
async function build(scope, config, next) {
    ...
    if (exposeInternalsIdx !== -1)
        process.execArgv.splice(exposeInternalsIdx, 1);
    try {
        switch (next.version) {
            case 14:
                await next.build({
        ...
        if (exposeInternalsIdx !== -1)
            process.execArgv.splice(exposeInternalsIdx, 0, '--expose-internals');
    }
}
async function serve(scope, config, next) {
